<template>
  <div>
    ...
    <div class="mb-6 flex justify-between items-center px-1">
      <h1 class="text-2xl font-semibold text-gray-800 m-0">Tags</h1>
    ...
    <div class="flex flex-wrap -mx-3">
      ...
      <div class="w-full lg:w-1/3 px-3 mb-6">
        <div class="bg-white rounded shadow-sm border-t-4 border-[#6c757d] flex flex-col">
          <div class="p-4 border-b border-gray-100">
            <h3 class="text-lg font-medium text-gray-800 m-0">{{ editingId ? 'Edit Tag' : 'Create Tag' }}</h3>
          </div>
          <div class="p-4">
            <form @submit.prevent="saveItem">
              <div class="mb-4">
                <label class="block text-gray-700 text-sm font-bold mb-2">Tag Name</label>
                <input v-model="form.name" placeholder="Enter tag name" required class="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:border-[#6c757d] focus:ring-1 focus:ring-[#6c757d] text-gray-900 bg-white" />
              </div>
              <div class="flex items-center">
                <button type="submit" class="bg-[#6c757d] text-white px-4 py-2 rounded text-sm hover:bg-gray-700 transition font-medium">{{ editingId ? 'Update Tag' : 'Submit' }}</button>
      ...
      <div class="w-full lg:w-2/3 px-3 mb-6">
        <div class="bg-white rounded shadow-sm border-t-4 border-gray-300 flex flex-col overflow-hidden">
           <div class="p-4 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
            <h3 class="text-lg font-medium text-gray-800 m-0">Tag List</h3>
             <div class="relative">
                 <input type="text" placeholder="Search" class="pl-3 pr-8 py-1.5 border border-gray-300 rounded text-sm focus:outline-none focus:border-[#6c757d] w-56 text-gray-900 bg-white placeholder-gray-400"/>
            ...
            <table class="w-full text-left text-sm whitespace-nowrap">
              <thead class="bg-gray-50 border-b border-gray-200 text-gray-600 font-semibold uppercase text-xs">
                <tr>
                  ...
                  <th class="px-5 py-3">Tag Name</th>
                  <th class="px-5 py-3 w-32 text-center">Actions</th>
                </tr>
              </thead>
              <tbody class="text-gray-800">
                <tr v-if="items.length === 0">
                    <td colspan="3" class="px-5 py-8 text-center text-gray-500">No tags found. Create one to see it here.</td>
                </tr>
                <tr v-for="item in items" :key="item.id" class="border-b border-gray-100 hover:bg-gray-50/50 transition-colors">
                  <td class="px-5 py-3 text-center align-middle text-gray-600">{{ item.id }}</td>
                  <td class="px-5 py-3 align-middle font-medium">
                    <span class="bg-gray-100 text-gray-700 py-1 px-3 rounded-full text-xs border border-gray-200">#{{ item.name }}</span>
                  </td>
                  <td class="px-5 py-3 align-middle text-center">
                    <button @click="editItem(item)" class="text-gray-600 hover:text-gray-800 bg-gray-100 hover:bg-gray-200 p-1.5 rounded transition mr-2 inline-flex" title="Edit">
