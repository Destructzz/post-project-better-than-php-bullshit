<template>
  <div>
    ...
    <div class="mb-6 flex justify-between items-center px-1">
      <h1 class="text-2xl font-semibold text-gray-800 m-0">Authors</h1>
    ...
    <div class="flex flex-wrap -mx-3">
      ...
      <div class="w-full lg:w-1/3 px-3 mb-6">
        <div class="bg-white rounded shadow-sm border-t-4 border-[#17a2b8] flex flex-col">
          <div class="p-4 border-b border-gray-100">
            <h3 class="text-lg font-medium text-gray-800 m-0">{{ editingId ? 'Edit Author' : 'Create Author' }}</h3>
          </div>
          <div class="p-4">
            <form @submit.prevent="saveItem">
              <div class="mb-4">
                <label class="block text-gray-700 text-sm font-bold mb-2">Author Name</label>
                <input v-model="form.name" placeholder="Enter name" required class="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:border-[#17a2b8] focus:ring-1 focus:ring-[#17a2b8] text-gray-900 bg-white" />
              </div>
              <div class="mb-4">
                <label class="block text-gray-700 text-sm font-bold mb-2">Email Address</label>
                <input type="email" v-model="form.email" placeholder="Enter email" required class="w-full px-3 py-2 border border-gray-300 rounded text-sm focus:outline-none focus:border-[#17a2b8] focus:ring-1 focus:ring-[#17a2b8] text-gray-900 bg-white" />
              </div>
              <div class="flex items-center">
                <button type="submit" class="bg-[#17a2b8] text-white px-4 py-2 rounded text-sm hover:bg-cyan-700 transition font-medium">{{ editingId ? 'Update Author' : 'Submit' }}</button>
      ...
      <div class="w-full lg:w-2/3 px-3 mb-6">
        <div class="bg-white rounded shadow-sm border-t-4 border-gray-300 flex flex-col overflow-hidden">
           <div class="p-4 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
            <h3 class="text-lg font-medium text-gray-800 m-0">Author List</h3>
             <div class="relative">
                 <input type="text" placeholder="Search" class="pl-3 pr-8 py-1.5 border border-gray-300 rounded text-sm focus:outline-none focus:border-[#17a2b8] w-56 text-gray-900 bg-white placeholder-gray-400"/>
            ...
            <table class="w-full text-left text-sm whitespace-nowrap">
              ...
              <tbody class="text-gray-800">
                <tr v-if="items.length === 0">
                    <td colspan="4" class="px-5 py-8 text-center text-gray-500">No authors found. Create one to see it here.</td>
                </tr>
                <tr v-for="item in items" :key="item.id" class="border-b border-gray-100 hover:bg-cyan-50/50 transition-colors">
                  ...
                  <td class="px-5 py-3 align-middle text-gray-600 truncate max-w-xs">{{ item.email }}</td>
                  <td class="px-5 py-3 align-middle text-center">
                    <button @click="editItem(item)" class="text-[#17a2b8] hover:text-cyan-700 bg-cyan-50 hover:bg-cyan-100 p-1.5 rounded transition mr-2 inline-flex" title="Edit">
