<template>
  <div class="max-w-4xl mx-auto my-8 p-8 bg-white rounded-lg shadow-sm">
    ...
    <div class="p-6 bg-gray-100 rounded-lg mb-8">
      <h3 class="text-xl font-semibold mb-4">{{ editingId ? 'Edit Category' : 'Create Category' }}</h3>
      <form @submit.prevent="saveItem" class="flex flex-col gap-4">
        <input v-model="form.name" placeholder="Category Name" required class="p-3 border border-gray-300 rounded bg-white text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500" />
        <textarea v-model="form.description" placeholder="Category Description" required rows="3" class="p-3 border border-gray-300 rounded bg-white text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500"></textarea>
    ...
    <div>
      <div v-if="items.length === 0" class="text-center text-gray-500 py-8">No categories found.</div>
      <div v-for="item in items" :key="item.id" class="p-6 border border-gray-200 rounded-lg flex justify-between items-center mb-4 bg-white hover:shadow-md transition-shadow">
        <div>
          <strong class="text-lg block text-gray-900">{{ item.name }}</strong>
